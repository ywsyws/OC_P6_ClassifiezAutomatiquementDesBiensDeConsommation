...
def plot_stacked_barchart(df:pd.DataFrame, cat1:str, cat2:str):
    ...
    df_cluster = (
        df[[cat1, cat2]]
        .groupby([cat1, cat2])[cat1]
        .count()
        .reset_index(name="count")
    )
    # plot stacked bar chart
    df_cluster = df_cluster.pivot_table(
        index=cat1, columns=cat2, values="count"
    )
    df_cluster.plot(kind="bar", stacked=True, figsize=(10, 10), rot=10, colormap="Dark2")
...
def plot_clusters(x: str, y: str, data: pd.DataFrame, hue: str, title: str = None, alpha:float=0.3):
    """
